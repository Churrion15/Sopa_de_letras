document.addEventListener('DOMContentLoaded', () => {
    ...
    const wordsToFind = [
        "EMULADOR", "GASTOS", "GITHUB", "INGRESAR", "SDK", "TYPESCRIPT",
        "EXPO", "GIT", "GRAFICA", "JAVASCRIPT", "TERMINAL",
    ...
    function generateGrid() {
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                ...
                // Eventos táctiles
                cell.addEventListener('touchstart', startSelectionTouch, { passive: false }); // passive: false para preventDefault
    ...
    function startSelectionTouch(e) {
        ...
        document.addEventListener('touchmove', duringSelectionTouch, { passive: false });
    ...
    function addCellToSelection(cell) {
        ...
        if (cell && cell.classList.contains('grid-cell') && !selectedCells.includes(cell)) {
            ...
            } else { // Si es la primera celda en la selección
                selectedCells.push(cell);
                cell.classList.add('selected');
            }
        }
    ...
    function checkWord() {
        ...
        for (let i = 0; i < wordsToFind.length; i++) {
            ...
            if ((currentWord === word || reversedWord === word) && !foundWords.has(word)) {
                found = true;
                foundWordText = word;
                break;
            }
    ...
    function playSound() {
        if (foundSound) {
            ...
            foundSound.play().catch(e => console.error("Error al reproducir el sonido:", e));
        }
    }

    function markWordAsFoundInList(word) {
        const normalizedWordId = `word-${word.toUpperCase().replace(/[^A-Z0-9]/g, '')}`;
    ...
    function endGame() {
        ...
        if (seconds > 0 || totalSeconds === 0) { // Muestra 0 segundos si el tiempo es muy corto
            timeString += `${seconds} segundo${seconds === 1 ? '' : 's'}`;
        }
        if (timeString === '') { // Para casos donde el tiempo es 0 o muy pequeño
